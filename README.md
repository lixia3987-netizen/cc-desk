...
## 源码新增：CLI 更新、上下文用量、斜杠命令与字体设置

以下功能已加入源码，尚未包含在 v0.3.0 安装包中。
...
- 图形化聊天输入区显示 **Context 占比、已用 token 与窗口容量**，展开可查看模型、更新时间和最近压缩记录。
...
- 默认使用**系统字体**；打开设置后读取已安装的字体，支持搜索、刷新，并分别用于聊天和菜单。系统字体直接使用本机安装，无需复制或重新导入。
...
| 字体与分类设置（主分支新增） | 聊天 / 菜单字体和字号独立配置、系统字体读取 / 搜索 / 选择、本机导入与移除；左侧五类菜单，右侧配置即时预览 |
