...
- **独立工具窗口**：右侧常驻「上下文 / 变更 / 工作流 / 诊断」竖向工具栏，点击只显示对应窗口，再点当前按钮即可隐藏。切换工具不会一次展开所有面板，隐藏后保留工具入口和当前输入，不重启终端。
...
| 面板布局 | 右侧常驻竖向工具栏；上下文、变更、工作流、诊断按按钮独立显隐，同侧显示一个窗口；记住当前工具或全部隐藏状态 |
...
| Worktree | 项目内或统一目录、按项目和名称创建；快进合并和安全清理；删除会话时可在提醒确认后直接删除隔离目录，无须满足 Git 清理条件 |
...
右侧采用类似 [IntelliJ IDEA 工具窗口](https://www.jetbrains.com/help/idea/tool-windows.html) 的常驻竖向工具栏。「上下文 / 变更 / 工作流 / 诊断」各有独立按钮：点击隐藏的工具只显示对应窗口，点击另一个工具切换窗口，再点当前工具或窗口标题栏的 × 即可隐藏内容。工具栏始终保留，内容收起后让出聊天空间；同侧一次显示一个完整窗口，不会联动展开整组面板。

在工具窗口内按 Shift+Esc 也可隐藏当前窗口。隐藏和切换工具会保留当前会话中未保存的输入，不重启正在运行的终端。当前工具或全部隐藏状态在本机记忆，切换会话或重启后沿用；旧版多面板布局会迁移为一个可见工具或全部隐藏。Git 审阅与工作流草稿沿用各自的会话存储。未保存的模型、权限等运行配置仍需点击「保存配置」，不会在切换会话或重启后自动保存。
...
如确定不再需要隔离目录中的文件，可选择「删除会话并强制删除隔离目录」，阅读提醒、核对目录路径后点击确认，无须输入确认文字。应用会自动停止使用此目录的会话，直接递归删除整个隔离目录；未提交、未跟踪、被忽略的文件和嵌套仓库均包含在删除范围内，不再检查合并状态、Git 锁、所有权或其他会话依赖。来源项目及其中保存的分支和提交保留，嵌套仓库内的分支和提交则随目录删除。

`.git` 丢失、损坏、来源仓库失效或目录已经不存在，均不阻止强制删除。目录删除后会尝试清理该 worktree 的 Git 登记；登记清理失败不影响删除结果，也不批量清理其他登记。共用被删除目录的其他会话会停止并归档，保留历史；来源目录和其他隔离目录中的会话继续运行。文件系统删除失败时保留会话供重试；若目录已删除而记录清理失败，可再次仅删除会话记录。
...
状态损坏时保留原文件并报错。恢复备份前先关闭应用并保留损坏文件副本。Worktree 快进合并和安全清理只处理具有可信所有权记录的目录；用户确认后的强制删除直接处理会话登记的隔离目录。
