...
| 面板布局 | 上下文、变更、工作流、诊断独立打开、关闭或折叠；宽窗口双列、窄窗口纵向自动排列，记住开关状态；顶栏可整体收起 |
...
右侧工具栏中的「上下文 / 变更 / 工作流 / 诊断」是独立开关，可以同时打开多个面板。每个面板的标题可折叠内容，右上角 × 只关闭该面板；关闭全部面板后仅保留工具栏。根据窗口可用宽度，多个面板自动采用双列或纵向排列，各自内容可滚动。

会话顶部最右侧的按钮可整体收起或展开右侧面板。关闭、折叠和布局调整会保留当前会话中未保存的输入，不重启正在运行的终端。面板的打开、折叠及整体显示状态在本机记忆，切换会话或重启后沿用；Git 审阅与工作流草稿沿用各自的会话存储。未保存的模型、权限等运行配置仍需点击「保存配置」，不会在切换会话或重启后自动保存。
