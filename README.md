...
刷新由 CLI 事件驱动，不定时轮询或自动发送 `/context`。手动报告后，普通对话继续更新输入用量并保留已知容量；后续事件缺少用量时保留上次有效读数和测量时间。报告的模型展示名称与请求中的模型标识不同，不会导致容量丢失。停止进程和重启应用保留已保存读数；确认切换模型或 `/clear` 后清除旧读数，压缩完成后保留容量并等待新请求用量。
