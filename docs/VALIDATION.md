...
- 本地环境禁止桌面测试所需的 socket，真实窗口验证转由项目原有手动三平台工作流执行。此处先记录已完成的本地结果，三平台结果随后补充；本轮尚未发布新的 Release。
- 首轮 CI 在新增字体测试的模块加载阶段发现 `import.meta.url` 与 Playwright CommonJS 转译的冲突，已改用项目根目录路径；实际包验证另检查三款字体的包内加载和完整许可证。
