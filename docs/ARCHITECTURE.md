...
Worktree 所有权记录写入该 worktree 的 Git 私有目录。只允许快进合并；安全清理要求记录有效、无使用中的会话、已合并且目录干净，包含忽略数据检查，不使用 force 或删除分支。

删除会话的显式强制清理以确认时的会话 ID 和登记路径绑定目标，在提醒后直接执行递归目录删除。此路径不发现 Git 根目录、不检查所有权、合并、脏文件、Git 锁或嵌套仓库，不修复 `.git`。文件系统目标边界仅排除根目录、来源/项目目录及 Git 元数据目录；末级符号链接只删除链接本身。Git 登记清理在目录删除成功后针对该路径尽力执行，失败不否定删除结果，不批量 prune、不删除来源仓库分支。

强制删除锁定目标路径并自动停止目录内的会话，取消工作流、暂停队列，等待已经进入的启动/发送和进程释放，避免删除期间重新启动。终端 `stopAndWait` 按会话传播异步进程及启动资源清理失败，不把根进程退出当作全部清理成功，也不因无关会话的历史失败阻止删除。来源目录及其他 worktree 中的活动会话不阻止此操作。共用被删除目录的其他记录归档并保留历史；目录删除与会话记录删除分步记录，后续历史或附件清理失败时可重试删除记录。
