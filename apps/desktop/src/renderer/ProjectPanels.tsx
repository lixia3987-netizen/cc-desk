...
import { Check, File, Folder, GitBranch, Loader2, RefreshCw, Search, X } from 'lucide-react';
...
export function GitPanel({session,onError,onReview,draft,onDraft,visible=true}:{session:Session;onError:(error:unknown)=>void;onReview:(text:string)=>boolean;draft:GitReviewDraft;onDraft:UpdateDraft<GitReviewDraft>;visible?:boolean}) {
  ...
  return <div className="panel-content"><div className="panel-heading"><strong><GitBranch size={14}/> {changes?.branch||'Git 变更'}</strong><button className="icon-button" title="刷新 Git 状态" disabled={refreshing} onClick={()=>void refresh()}><RefreshCw size={14} className={refreshing?'spin':''}/></button></div>
    ...
    {session.worktree&&tree&&!tree.owned&&<div className="worktree-panel"><h4>隔离目录</h4>{tree.reasons.map((reason,i)=><p className="panel-note" key={i}>{reason}</p>)}<p className="panel-note">也可在“上下文”中仅删除会话并保留目录。</p><p className="panel-note">{tree.path}</p><button className="secondary compact" onClick={()=>void window.desktop.openFolder(session.id).catch(onError)}><Folder size={13}/>打开隔离目录</button></div>}
    {tree?.owned&&<div className="worktree-panel"><h4>隔离工作区</h4><p className="panel-note">{tree.branch} → {tree.baseBranch??'原项目分支'}</p><p>{tree.clean?'工作区干净':'存在未提交改动'} · {tree.merged?'已合入基础分支':'尚未合入'}</p>{tree.reasons.filter(reason=>!tree.cleanupReasons.includes(reason)).map((reason,i)=><p className="panel-note" key={i}>{reason}</p>)}{!tree.canCleanup&&<div id={'cleanup-blockers-'+session.id}><p className="panel-note">暂时无法清理隔离目录：</p>{tree.cleanupReasons.map((reason,i)=><p className="panel-note" key={i}>{reason}</p>)}<p className="panel-note">也可在“上下文”中仅删除会话并保留目录。</p></div>}<p className="panel-note">{tree.path}</p><button className="secondary compact" onClick={()=>void window.desktop.openFolder(session.id).catch(onError)}><Folder size={13}/>打开隔离目录</button><div className="panel-actions"><button className="secondary compact" disabled={busy||!tree.canMerge} onClick={()=>setConfirm('merge')}>合并到原项目</button><button className="secondary compact danger" disabled={busy||!tree.canCleanup} aria-describedby={!tree.canCleanup?'cleanup-blockers-'+session.id:undefined} onClick={()=>setConfirm('cleanup')}>清理隔离目录</button></div>{confirm&&<div className="action-confirm"><p>{confirm==='merge'?'将已提交变更快进合并到原项目。请确认这是你要交付的变更。':'将移除这个已合并且干净的隔离目录，保留 Git 分支。'}</p><button className="secondary compact" disabled={busy} onClick={()=>setConfirm(undefined)}>取消</button><button className="primary compact" disabled={busy} onClick={()=>void act()}>{busy?<Loader2 size={13} className="spin"/>:<Check size={13}/>}确认{confirm==='merge'?'合并':'清理'}</button></div>}</div>}
