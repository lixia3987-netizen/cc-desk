...
interface Props {
  state: AppState; historyAvailable: boolean; paletteQuery: string; setPaletteQuery: (value: string) => void;
  openNew: OpenNew; openHistory: () => Promise<void>; onSettings: () => void; onSelect: (session: Session) => void;
}

export function CommandPalette({ state, historyAvailable, paletteQuery, setPaletteQuery, openNew, openHistory, onSettings, onSelect }: Props) {
  return <>
    ...
    <div className="palette-results">{['新建会话', '导入 CLI 历史', '设置与连接'].filter(name => name.includes(paletteQuery)).map(name =>
      <button key={name} disabled={name === '导入 CLI 历史' && !historyAvailable} onClick={() => { if (name === '新建会话') openNew(); else if (name === '导入 CLI 历史') void openHistory(); else { onSettings(); } }}>{name}<ChevronRight size={14} />
