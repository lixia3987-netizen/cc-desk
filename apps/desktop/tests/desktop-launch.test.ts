...
import { desktopRoot, repositoryRoot, documentationPath, testResultsPath } from './helpers/paths';

const root = fileURLToPath(new URL('../', import.meta.url));
const runner = path.join(root, 'scripts', 'run-desktop-tests.mjs');
const require = createRequire(import.meta.url);

test('workspace paths and desktop identity remain independent of the root workspace manifest', () => {
  const manifest = JSON.parse(fs.readFileSync(path.join(desktopRoot, 'package.json'), 'utf8'));
  assert.equal(manifest.name, 'claude-workbench');
  assert.equal(manifest.productName, 'Claude Workbench');
  assert.equal(manifest.build.appId, 'io.local.claude-workbench');
  assert.equal(manifest.main, 'dist/main/index.cjs');
  assert.equal(desktopRoot, path.join(repositoryRoot, 'apps', 'desktop'));
  assert.equal(documentationPath('screenshots'), path.join(repositoryRoot, 'docs', 'screenshots'));
  assert.equal(testResultsPath('packaged'), path.join(repositoryRoot, 'test-results', 'packaged'));
  assert.deepEqual(electronLaunchArgs(undefined, 'win32'), [desktopRoot]);
});
...
test('desktop runner preserves filter arguments, desktop cwd and Xvfb child exit status', { skip: process.platform !== 'linux' }, () => {
  ...
  try {
    ...
    assert.deepEqual(JSON.parse(fs.readFileSync(log, 'utf8')), {
      cwd: root.replace(/[\\/]$/, ''),
      args: ['-a', '-s', '-screen 0 1920x1080x24 -nolisten tcp', process.execPath, require.resolve('@playwright/test/cli'), 'test', '--config', path.join(desktopRoot, 'playwright.config.ts'), ...filters],
    });
