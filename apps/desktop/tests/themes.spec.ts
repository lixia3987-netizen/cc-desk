...
test('themes: five readable themes preserve a live terminal, cancel previews and survive restart', async () => {
  ...
  await fs.mkdir(documentationPath('themes'), { recursive: true });
  ...
  try {
    ...
    for (const choice of choices) {
      ...
      if (choice.id === 'forest') {
        ...
        await page.screenshot({ path: documentationPath('themes', 'settings.png') });
      ...
      await page.screenshot({ path: documentationPath('themes', `${choice.id}.png`) });
