...
(async()=>{
 if(process.platform==='linux'&&!process.env.DISPLAY?.trim())throw new Error('Linux 界面截图需要 X11 DISPLAY，请安装 xvfb/xauth 后运行 xvfb-run -a node apps/desktop/scripts/capture-layout.cjs。');
 const desktopRoot=path.resolve(__dirname,'..'),repoRoot=path.resolve(__dirname,'../../..'),root=path.join(repoRoot,'docs/screenshots'),workspace=await fs.mkdtemp(path.join(os.tmpdir(),'cc-desk-layout-'));
 ...
 let app;
 try{
  app=await electron.launch({args:[desktopRoot,...(process.platform==='linux'?['--no-sandbox','--ozone-platform=x11','--disable-gpu']:[])],cwd:desktopRoot,env:{...process.env,WORKBENCH_TEST_MODE:'1',WORKBENCH_DATA_DIR:data}});
