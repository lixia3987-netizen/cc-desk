...
const done = result => output({ type:'result', subtype:'success', session_id:session, result, usage:{input_tokens:900000}, modelUsage:{'fixture-model':{contextWindow:200000}}, num_turns:1 });
readline.createInterface({input:process.stdin}).on('line', line => {
  ...
  output({type:'system',subtype:'init',session_id:session,model:'fixture-model',slash_commands:commands.map(item=>item.name),skills:['team:review']});
  ...
  if (text==='/context') { output({type:'assistant',context_usage:{model:'fixture-model',total_tokens:30000,raw_max_tokens:200000},message:{id:'report-'+turn,content:[{type:'text',text:'详细上下文报告'}]}});done('详细上下文报告');return; }
  output({type:'stream_event',event:{type:'message_start',message:{id:'message-'+turn,model:'fixture-model',usage:{input_tokens:2000,cache_read_input_tokens:9000,cache_creation_input_tokens:1000}}}});
  output({type:'assistant',message:{id:'message-'+turn,model:'fixture-model',usage:{input_tokens:2000,cache_read_input_tokens:9000,cache_creation_input_tokens:1000},content:[{type:'text',text:'完成请求'}]}});
