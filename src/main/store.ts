  ...
  constructor(readonly directory: string, private options: { writeDelayMs?: number; onError?: (error: Error) => void } = {}) {
    ...
    if (fs.existsSync(this.file)) {
      ...
      try {
        ...
        // Migrate only this retired built-in. Other invalid workspace data must
        // still fail validation; new settings cannot select the removed font.
        for (const key of ['chatFontFamily', 'uiFontFamily'] as const) {
          if (saved?.settings?.[key] === 'noto-serif-sc') {
            saved.settings[key] = DEFAULT_TYPOGRAPHY[key];
            this.dirty = true;
          }
        }
