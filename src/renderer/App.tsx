  ...
  return <div className="app">
    <aside className="sidebar">
      ...
      <div className="session-list">
        {sessions.map(s=><button key={s.id} className={`session-row ${activeId===s.id?'active':''}`} onClick={()=>selectSession(s.id)}>
    ...
    <main className="workspace">
      ...
      {!active?<div className="welcome">
        ...
        <div className="session-content"><section className="terminal-section"><div className="terminal-toolbar"><span><TerminalSquare size={14}/>{structured?'结构化对话':'交互终端'}</span><span title={active.cwd}>{active.cwd}</span><button className="icon-button" title="打开工作目录" onClick={()=>void perform(()=>window.desktop.openFolder(active.id))}><FolderOpen size={14}/></button></div>
          ...
          {inspectorTab==='context'&&<div className="panel-content"><div className="section-label">会话上下文<Activity size={14}/></div><div className="detail-block"><label>项目</label><strong>{project?.name}</strong><label>运行方式</label><strong>{structured?'结构化对话':active.kind==='shell'?'系统 Shell':'Claude Code 终端'}</strong><label>创建时间</label><strong>{time(active.createdAt)}</strong>{active.kind==='claude'&&<><label>Claude 会话 ID{active.identityPending?' · 等待同步':''}</label><button className="id-copy" title="复制会话 ID" onClick={()=>void perform(async()=>{await navigator.clipboard.writeText(active.claudeId);setNotice('会话 ID 已复制');})}>{active.claudeId.slice(0,18)}…<Copy size={12}/></button></>}</div>
    ...
    {modal&&<Dialog className={modal==='settings'?'wide':''} onClose={()=>setModal(null)} closeDisabled={busy} label={modal==='new'?'新建会话':modal==='settings'?'设置与连接':modal==='rename'?'重命名会话':modal==='palette'?'命令面板':'导入 CLI 历史'}><button className="icon-button close-modal" disabled={busy} aria-label="关闭弹窗" onClick={()=>setModal(null)}><X size={20}/></button>
      ...
      {modal==='settings'&&draftSettings&&<><div className="eyebrow">PREFERENCES</div><h2>设置与连接</h2><p>检测已安装的 Claude Code，使用本机账户与配置。</p><form onSubmit={event=>{event.preventDefault();void perform(async()=>{await window.desktop.saveSettings(draftSettings);await refresh();setNotice('设置已保存并重新检测');});}}>
        ...
        <div className={`connection-box ${cap.available?'connected':''}`}><div><span className={`dot ${cap.available?'running':'error'}`}/><strong>{cap.available?cap.version:'未检测到 CLI'}</strong></div><p>{cap.available?cap.executable:cap.error||'保存设置后自动检测 CLI。'}</p>{cap.available&&<small>可用强度：{cap.efforts.filter(x=>x!=='default').join(' / ')||'跟随 CLI'}</small>}</div>
