...
import type { ChatApproval, ChatSnapshot } from '../shared/chat';
...
function inline(text:string) {
  return text.split(/(\x60[^\x60]+\x60|\*\*[^*]+\*\*)/g).map((part,index) => part.startsWith('\x60') && part.endsWith('\x60') ? <code key={index}>{part.slice(1,-1)}</code> : part.startsWith('**') && part.endsWith('**') ? <strong key={index}>{part.slice(2,-2)}</strong> : part);
}
/** Small Markdown subset rendered as React nodes; no HTML interpretation. */
export function MessageText({text}:{text:string}) {
  return <div className="message-markdown">{text.split(/(\x60\x60\x60[\s\S]*?(?:\x60\x60\x60|$))/g).map((block,index) => {
    if(block.startsWith('\x60\x60\x60')) {
      const body=block.endsWith('\x60\x60\x60')?block.slice(3,-3):block.slice(3), newline=body.indexOf('\n');
      return <div className="message-code" key={index}><span>{newline>=0?body.slice(0,newline)||'代码':'代码'}</span><pre><code>{newline>=0?body.slice(newline+1):body}</code></pre></div>;
    }
    return <div key={index}>{block.split('\n').map((line,i)=>/^#{1,4}\s/.test(line)?<h4 key={i}>{inline(line.replace(/^#{1,4}\s/,''))}</h4>:<div className="message-line" key={i}>{line?inline(line):<br/>}</div>)}</div>;
  })}</div>;
}

function ApprovalCard({approval,sessionId,onError}:{approval:ChatApproval;sessionId:string;onError:(error:unknown)=>void}) {
  const [answers,setAnswers]=useState<Record<string,string>>({}), [reason,setReason]=useState(''), [busy,setBusy]=useState(false);
  const respond=async(behavior:'allow'|'deny')=>{
    setBusy(true);
    try {await window.desktop.respondChat(sessionId,approval.requestId,{behavior,message:reason||undefined,answers:approval.kind==='question'?answers:undefined});}
...
export function ChatPane({session,draft,onDraft,onSent,onError,onAttach,onProjectFiles,attachments,onRemoveAttachment}:{
  session:Session;draft:string;onDraft:(value:string)=>void;onSent:(expectedDraft:string)=>void;onError:(error:unknown)=>void;onAttach:()=>void;onProjectFiles:()=>void;attachments:string[];onRemoveAttachment:(path:string)=>void;
}) {
  ...
  const load=useCallback(async()=>{const seq=++request.current;const value=await window.desktop.chatSnapshot(session.id);if(mounted.current&&seq===request.current)setSnapshot(value);},[session.id]);
  useEffect(()=>{
...
const task=snapshot?.taskState??session.taskState??'idle', running=sending||busyTask(task);
  const send=async()=>{
    if(!draft.trim()||running||session.archived)return;
    if(draft.length>60000){onError(new Error('单次提示词请控制在 60,000 个字符以内。'));return;}
    setSending(true);setFollow(true);
    try{
      const result=await window.desktop.sendChat(session.id,draft.trim(),attachments);
      if(result.success)onSent(draft);
      if(result.success)attachments.forEach(onRemoveAttachment);
  ...
  return <div className="chat-pane">
    <div className="chat-scroll" ref={scroll} aria-label="对话记录" onScroll={e=>{const el=e.currentTarget;setFollow(el.scrollHeight-el.scrollTop-el.clientHeight<70);}}>
      ...
      {snapshot?.messages.map(message=>message.role==='tool'?<details key={message.id} className={'tool-card '+(message.isError?'has-error':'')}><summary><span className={'dot '+(message.isError?'error':'idle')}/><strong>{message.toolName??'工具结果'}</strong>{message.parentToolUseId&&<small>子任务</small>}<span>{message.isError?'失败':'查看详情'}</span></summary>{message.input&&<pre className="tool-input">{JSON.stringify(message.input,null,2)}</pre>}<MessageText text={message.text}/></details>:<article key={message.id} className={'chat-message '+message.role}><header>{message.role==='user'?'你':message.role==='assistant'?'Claude':'会话记录'}{message.parentToolUseId&&<small>子任务</small>}</header><MessageText text={message.text}/></article>)}
      {snapshot?.pending.map(approval=><ApprovalCard key={approval.requestId} approval={approval} sessionId={session.id} onError={onError}/>)}
    ...
    <div className="composer chat-composer">{attachments.length>0&&<div className="attachment-chips">{attachments.map(path=><span key={path} title={path}><Paperclip size={12}/>{path.split(/[\\/]/).pop()}<button className="icon-button" aria-label={'移除附件 '+path} onClick={()=>onRemoveAttachment(path)}><X size={12}/></button></span>)}</div>}
      <textarea aria-label="提示词编辑器" placeholder="描述任务… Ctrl / ⌘ + Enter 发送" value={draft} disabled={session.archived} onChange={e=>onDraft(e.target.value)} onKeyDown={e=>{if((e.metaKey||e.ctrlKey)&&e.key==='Enter'){e.preventDefault();void send();}}}/>
      <div><button className="icon-button" title="添加图片、PDF 或文件附件" aria-label="添加附件" disabled={running} onClick={onAttach}><Paperclip size={16}/></button><button className="icon-button" title="引用项目文件" aria-label="引用项目文件" onClick={onProjectFiles}><File size={16}/></button><span>草稿自动保存 · {attachments.length} 个附件</span>{running?<button className="secondary compact" onClick={()=>void window.desktop.interruptSession(session.id).catch(onError)}><Square size={12}/>中断</button>:<button className="primary compact" disabled={!draft.trim()||session.archived} onClick={()=>void send()}><CornerDownLeft size={14}/>发送任务</button>}</div>
    </div>
  </div>;
}
