...
interface Props {
  state: AppState; active: Session; structured: boolean; themeId: ThemeId;
...
export function SessionViewport({ state, active, structured, themeId, composer, onDraft, onSent, report, onClearError, children }: Props) {
  ...
  useEffect(() => {
    ...
    setMounted(ids => {
      const terminals = sessions.filter(s => s.execution.mode === 'terminal');
  ...
  const sendNative = async () => {
    ...
    if (!current || current.status !== 'running') { report(new Error('请先启动或恢复 Claude 终端。')); return; }
    if (current.terminalSync !== 'synced') { report(new Error('当前 CLI 尚未同步任务状态，请使用“粘贴到终端”并在终端确认发送。')); return; }
  ...
  return <section className="terminal-section">
    ...
    <div className="terminals" style={{ display: structured ? 'none' : undefined }}>{mounted.map(id => {
      const session = state.sessions.find(s => s.id === id); return session ? <div className="terminal-slot" key={id} style={{ display: id === activeId ? 'block' : 'none' }}>
        <TerminalPane session={session} themeId={themeId} settings={state.settings} active={id === activeId} onError={report} ref={handle => { if (handle) handles.current.set(id, handle); else handles.current.delete(id); }} />
      </div> : null;
    })}
      {!active.started && <div className="terminal-empty">
        <TerminalSquare size={30} />
        <h3>会话准备就绪</h3>
        <p>启动后，在这里与 {active.kind === 'shell' ? 'Shell' : 'Claude Code'} 直接交互。</p>{active.kind === 'agent' && <small>登录、信任目录与工具审批均在终端内完成</small>}</div>}
    </div>
    {active.kind === 'agent' && !structured && <>
      ...
      <div className="composer">
        ...
        <div>
          <span title="Ctrl / ⌘ + Enter 或 Shift + Enter 换行">{active.terminalSync === 'synced' ? 'Enter 发送 · Ctrl / ⌘ + Enter 换行' : '状态未同步：粘贴后请在终端确认并按 Enter 发送。'}</span>{active.terminalSync === 'synced' ? <button className="primary compact" disabled={active.status !== 'running' || activeBusy || nativeSending === active.id || active.archived || !composer.trim()} onClick={() => void sendNative()}>
            <CornerDownLeft size={13} />发送任务</button> : <button className="secondary compact" disabled={active.status !== 'running' || active.archived || !composer.trim()} onClick={() => { if (composer.length > 60000) { report(new Error('单次提示词请控制在 60,000 个字符以内。')); return; } const handle = handles.current.get(active.id); if (!handle) { report(new Error('终端尚未就绪，请稍后再试。')); return; } handle.paste(composer); handle.focus(); onDraft(''); }}>
