...
const { version, name, productName } = JSON.parse(await fs.readFile(path.join(root, 'package.json'), 'utf8'));
...
try {
  ...
  const report = path.join(root, 'test-results', 'packaged-manifest.json');
  ...
  run(process.execPath, [require.resolve('@playwright/test/cli'), 'test', '--config', 'playwright.packaged.config.ts'], {
    cwd: root, timeout: 600_000, env: { ...process.env, WORKBENCH_PACKAGED_TARGETS: manifest },
  });
